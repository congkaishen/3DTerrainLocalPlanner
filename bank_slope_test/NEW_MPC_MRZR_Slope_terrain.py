...
def defineSolver(init_states, goal, obslist, OCPParams):
    ...
    npar = 5
    OCPParams = {
        "XL":[-10, -5, -pi/2, -5, -pi/2, 0.5, -pi/9],
        "XU":[120, 5, pi/2, 5, pi/2, 9, pi/9],   
        "CL":[-0.35, -2.5],
        "CU":[0.35, 2.5],
        "XF_S":3.6,
        "OBS_SMX":1.0,
    ...
    par4 = SX.sym('par4')
    par5 = SX.sym('par5')
    parSX = vertcat(par1, par2, par3, par4, par5)
...
def main():
    ...
    T =  OCPParams["T"]
    goal = [110, 0.0]
    obslist = np.matrix([[50, -1, 2.5, 2.5],[70,1,2.5,2.5],[90, -2, 2.5, 2.5]])
...
###########################
    ...
    savemat("Result_Mat\Slope_Ter.mat", KeySolutions)
